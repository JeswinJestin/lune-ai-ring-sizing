import React from 'react';

type IconWrapperProps = React.SVGProps<SVGSVGElement> & {
    children: React.ReactNode;
};

const IconWrapper = ({ children, ...props }: IconWrapperProps) => (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" xmlns="http://www.w3.org/2000/svg" {...props}>
        {children}
    </svg>
);

export const GithubIcon = (props: React.SVGProps<SVGSVGElement>) => (
    <IconWrapper {...props}>
        <path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"></path>
    </IconWrapper>
);

export const BehanceIcon = (props: React.SVGProps<SVGSVGElement>) => (
    <IconWrapper {...props}>
        <path d="M17.9 10.4c0-1.4-.8-2.5-2.5-2.5H9v10h6.1c1.8 0 2.8-1.1 2.8-2.8v-1.8c0-.6-.2-1-.5-1.3.4-.3.5-.8.5-1.3v-1.3zM11 11.8h4c.6 0 1 .4 1 .9s-.4.9-1 .9h-4v-1.8zm0 3.8h4.4c.7 0 1.1.4 1.1.9s-.4 1-1.1 1H11v-1.9zM22 7H2v2h20V7z"></path>
    </IconWrapper>
);

export const LinkedInIcon = (props: React.SVGProps<SVGSVGElement>) => (
    <IconWrapper {...props}>
        <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path>
        <rect x="2" y="9" width="4" height="12"></rect>
        <circle cx="4" cy="4" r="2"></circle>
    </IconWrapper>
);
